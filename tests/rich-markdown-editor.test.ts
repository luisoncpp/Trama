...
import type { EditorSession } from '../src/features/project-editor/project-editor-types'
...
describe('RichMarkdownEditor', () => {
  ...
  function createSessionCapture() {
    const holder = { current: null as EditorSession | null }
    return {
      onSessionReady: (session: EditorSession | null) => {
        holder.current = session
      },
      flush: () => holder.current?.flush() ?? null,
    }
  }
    ...
    act(() => {
      render(
        h(RichMarkdownEditor, buildEditorProps({
          ...
          onSessionReady: sessionCapture.onSessionReady,
        })),
        container,
      )
    })
  ...
  describe('IMAGE_PLACEHOLDER round-trip regression', () => {
    ...
    it('preserva imagen en markdown tras pegar imagen base64', async () => {
      ...
      // onChange receives editor-internal placeholder markdown
      expect(lastMarkdown).toContain('<!-- IMAGE_PLACEHOLDER:img_0 -->')
      expect(lastMarkdown).not.toContain(TINY_PNG_1X1.substring(0, 20))
    ...
    it('renderiza imagen rota como emoji y preserva su markdown original al serializar', async () => {
      ...
      expect(lastMarkdown).toContain('TRAMA_BROKEN_IMAGE:')
      expect(lastMarkdown).not.toContain('![cover](res/missing-image.png)')
    })
  })
