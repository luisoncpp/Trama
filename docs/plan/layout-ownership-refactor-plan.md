...
[sidebar-width-responsive-collapse-sync.md](docs/lessons-learned/sidebar-width-responsive-collapse-sync.md).
...
[focus-mode-css-grid-display-none-auto-place.md](docs/lessons-learned/focus-mode-css-grid-display-none-auto-place.md):
...
[map-editor-rendering-quirks.md](docs/lessons-learned/map-editor-rendering-quirks.md) lesson is a
...
> [layout-component-prevents-micro-file-proliferation.md](docs/lessons-learned/layout-component-prevents-micro-file-proliferation.md),
...
[START-HERE.md:78](docs/START-HERE.md#L78)) push toward extraction, but
[layout-component-prevents-micro-file-proliferation.md](docs/lessons-learned/layout-component-prevents-micro-file-proliferation.md)
...
- **Risk:** medium — a missed property during consolidation collapses the editor to 0px.
  **Verify** (real Chromium, not JSDOM — per
  [map-editor-rendering-quirks.md](docs/lessons-learned/map-editor-rendering-quirks.md)) that both
...
[css-patch-corruption.md](docs/lessons-learned/css-patch-corruption.md) hazard around CSS edits.
...
- **Section 9 = Quill, untouchable structure:** never inject DOM into `.ql-editor`
  ([START-HERE.md:85](docs/START-HERE.md#L85)); these rules theme Quill's own DOM and must remain
...
- Focus overlay/highlight rules tied to `::highlight(trama-focus-scope)` and `.ql-editor` pseudo
  ...
  ([focus-mode-centered-scroll-spacers.md](docs/lessons-learned/focus-mode-centered-scroll-spacers.md)).
...
> JSDOM has no layout engine ([map-editor-rendering-quirks.md](docs/lessons-learned/map-editor-rendering-quirks.md)):
...
- **Tests:** existing focus-mode tests (the CSS-vars-on-activate test referenced in
  [focus-mode-css-vars-sync-init.md](docs/lessons-learned/focus-mode-css-vars-sync-init.md)) must
...
- **Tests:** existing split-pane tests
  ([split-pane-coordination.md](docs/architecture/split-pane-coordination.md) suite) pass unchanged;
...
Per [START-HERE.md](docs/START-HERE.md) anti-forget checks, the following are required **in the same
task** as implementation:

1. **`docs/live/file-map.md`** — add the two new files (`layout/layout-metrics.ts`,
   `layout/use-sidebar-layout.ts`) with ownership notes ("single source for layout constants /
   effective sidebar collapse").

2. **`docs/lessons-learned/sidebar-width-responsive-collapse-sync.md`** — update the "Invariant
...
3. **`docs/lessons-learned/README.md`** — add a one-line entry pointing at a new lesson
...
4. **New `docs/architecture/layout-ownership.md`** (small) — the canonical map: which module owns
   ...
   `docs/architecture/README.md` and from the sidebar/focus/split architecture docs.

5. **`docs/architecture/sidebar-architecture.md`** and **`focus-mode-architecture.md`** — add a
...
7. **`docs/live/current-status.md`** — no feature change, but add a one-line note under a
