...
| `projected-state-vs-pane-target-state.md` | Projected UI aliases (`selectedPath`, `isDirty`) derive from active pane state; multi-step pane actions must use pane-targeted lookups, not projected values, or timing between activePane and async load causes stale checks | 2026-04-28 |
