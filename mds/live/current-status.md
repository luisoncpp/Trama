# Current Status

> **New conversation?** Open `mds/START-HERE.md` first — it routes you to the 3-4 files you actually need.

## Product state

Phase 2, Phase 3 (WS1–WS5), and Phase 4 WS1/wiki-tag-links, WS2/folder-operations, WS5/AI-import-export are complete.

**Implemented:** Electron + Vite + Preact shell, typed IPC (Zod/envelope), project open flow with folder-structure validation plus startup restore of the last valid project root, recursive markdown scan, read/save via repository layer, YAML frontmatter, `.trama.index.json` reconciliation, external-file-watch conflict detection, rich markdown visual editor (Quill-based), map documents with interactive markers (frontmatter `type: map`, image base layer, pan/zoom, marker create/edit/delete, tag-based navigation, sidebar creation flow that copies a chosen image into `res/` and starts with no markers), relationships charts (frontmatter `type: relationships`, character nodes with typed/colored/directional edges, reusable edge presets, labeled regions, pan/zoom, Add-character Auto tag, and Discord/Slack-style emoji decoration chips on nodes with an on-canvas curated emoji picker), sidebar (hierarchical tree, filter, create/rename/delete, collapse), workspace split mode (dual pane, drag divider, click-to-activate, persistence), per-pane session document history with browser-style back/forward (`Alt+Left`, `Alt+Right`, menu bar Back/Forward, toolbar Back), theme system (light/dark/system, WCAG AA, matchMedia sync), fullscreen/focus mode (line/sentence/paragraph scope dimming, CSS Highlights API + fallback), UX hardening (context menu controls, smart typography, Paste from Markdown, in-document Find), editor serialization debounce, spellcheck controls, Wiki tag links (TagIndexService, implicit matching, Ctrl+click navigation), folder rename/delete/move (end-to-end including DnD), drag-and-drop file reorder/move with corkboardOrder integration, AI import/export (clipboard pipeline, multi-file, replace/append modes), book export (multi-format: markdown/html/docx/epub/pdf with directives and images), ZuluPad import, revert changes button in editor toolbar (recarga desde disco, descarta cambios sucios), document zoom via Ctrl++/Ctrl+- (shared across twin panes), markdown image persistence via project-local `res/*.png` files with backward-compatible loading of legacy embedded-image documents and optional linked-image deletion on article remove, local Git history backend plus renderer integration for sidebar snapshots and pane-local revision preview/load UI, Help menu and singleton child Help window with local HTML content, theme synchronization, auto-opening on first project load, and dismissal preference persistence, and on-demand section word count calculation (Manuscript, Outline, Lore, Total) triggered from the Import/Export sidebar section with a modal display.
Template MVP is now implemented: `templates/` rail section for browsing/editing template markdown files, searchable template picker in create-article modal, silent `templates/` folder auto-creation on project open, `createFromTemplate` IPC for template-based document creation preserving frontmatter. Remaining WS3 items (placeholder substitution, "Save as Template" context menu, template preview form, tag-index exclusion, bundled default templates) are deferred.

Document contents navigation (slice 1) is now implemented: **Contents** rail section listing the active pane's H1–H3 headings as an indented, document-ordered index (ordinal identity; frontmatter/fence-aware parser; plain-text display), click-to-navigate with centered scroll + caret placement (works in read-only revision preview, never marks the pane dirty, targets only the active pane), live list updates on the editor serialization debounce via a narrow `DocumentContentsContext`, and spec §5.3 states (no document / no headings / map / relationships). Focused baseline: `npm run test -- tests/document-contents-parser.test.ts tests/document-contents-reveal.test.ts tests/sidebar-contents-panel.test.ts tests/project-editor-view-render-split.test.ts tests/sidebar-panels.test.ts`. Deferred slices (scroll-spy, quick-jump palette, drag-reorder) are documented in `mds/spec/document-contents-navigation-spec.md` §3.2; see `mds/plan/done/document-contents-navigation-implementation-plan.md`.

Contents labels are implemented for spacers and page breaks. The canonical backward-compatible syntax adds optional JSON-string metadata (`label="..."`) to existing comments. Labels are editable from a Contents-row pencil in writable panes, are invisible in the editor canvas and reader-facing book exports, and persist in source-oriented Copy as Markdown and AI exports. Filter toggles retain document-global ordinals; labeling a blank-line spacer converts it to an explicit spacer directive.

**Completed infrastructure:**
- `pane-isolation-plan-v2` — Módulo `pane/` fully encapsulates all pane mutation; `PaneWorkspace` is the exclusive mutation surface; Preact setters are injected via `usePaneWorkspace` factory hook; `markPaneSaved` is private; `saveNow` is awaitable (see `mds/plan/done/pane-isolation-plan-v2.md`, `mds/lessons-learned/awaitable-save-actions.md`).
- `useProjectEditor-keystroke-churn` — Issues #1–#4 completed end-to-end: (1) dirty signal split so already-dirty panes are no-ops, (2) stable `PaneWorkspace` identity, (3) memoized action groups, (4) narrowed shell/dialog subscriptions so typing only re-renders pane/editor surfaces. See `mds/plan/done/use-project-editor-keystroke-churn-plan.md`.
- `rich-editor-session-deepening` — Rich editor absorbed into a single `EditorSession` module per pane (lifecycle, serialization, external sync, layout directives, find, focus, tag overlay, zoom, toolbar). Callers use the typed `flush()` seam; old ref-mutation modules deleted. See `mds/plan/done/rich-editor-session-deepening-plan.md`.

## Maintenance notes

- Project editor layout ownership is now consolidated: `useSidebarLayout()` owns effective sidebar collapse/width, `.editor-workspace` is the sole sidebar-width writer, and `.editor-fill-column` names the shared editor height-fill contract.
- Project editor CSS is physically split under `src/styles/01-10-*.css`, imported in numeric order from `src/index.css` to preserve cascade order while making layout ownership inspectable by file.
- Theme architecture is now documented in `mds/architecture/theme-architecture.md`; dark-mode editor interaction states use the blue accent model instead of an accidental warm Quill-specific active token.

**Next step:** Folder move/reparent reintroduced in dedicated slice after merge.

- Document content session refactor implemented: image/broken-image phase orchestration now lives in `DocumentContentSession` (renderer) and `DiskContentAdapter` (main process); pane state is editor-internal end-to-end and `forIpcSave` is the single save-time hydration point.

## Reliability

- `npm run build` ✅ after document contents navigation slice 1 (2026-07-21).
- Full suite ✅: `powershell -ExecutionPolicy Bypass -File scripts/run-tests.ps1` — 117/117 files, 951/951 tests (report in `reports/test-report.txt`).
- `npm run lint` ✅ passing with 0 warnings/errors.
- Electron smoke test ✅ via the full suite; standalone `npm run test:smoke` was not run separately.

In sandboxed agent environments use the PowerShell script — see `mds/dev-workflow.md`.

## Known tradeoffs

- `sandbox: false` for preload stability.
- Index refresh now uses incremental updates for create/rename/delete/move/reorder; only save uses `updateCache`; full reconciliation still runs on initial open and cache miss.
- External change handling favors safety over convenience (prevents overwrite when dirty).
- PDF export uses HTML **PDF export segments** + Electron `printToPDF` + linear `pdf-lib` merge ([ADR 0004](../adr/0004-book-pdf-via-html-print-segments.md)); Times New Roman via `book-export-pdf-print.css` (`@page` A4, ~50pt margins). Legacy pdf-lib layout removed. CI tests mock the print surface; real export needs Electron. Cover/portada: unwrap `<p><img>` and cap image height to avoid blank leading pages — see `mds/lessons-learned/book-export-pdf-print-surface.md`.

## Phase 4

| Workstream | Status |
|---|---|
| WS1 — Wiki Tag Links | ✅ Complete |
| WS2 — Folder Operations | ✅ Complete (rename/delete/move + DnD + corkboardOrder) |
| WS3 — Templates | ✅ Complete (MVP: rail section + searchable picker + auto-create + createFromTemplate IPC) |
| WS4 — Corkboard | ❌ Cancelled |
| WS5 — AI Import/Export | ✅ Complete |
