...
| `quill-empty-paragraph-turndown-spacing-artifact.md` | Turndown emits whitespace artifacts for empty Quill paragraphs, while appending `\n` after a `BlockEmbed` matcher creates a real extra empty paragraph; normalize the former and never add the latter | 2026-07-22 |
...
| `find-match-refresh-must-not-steal-editor-focus.md` | Content-mutation match refresh must not call `keepFindFocus()`; gate overlay reveal on `isFindBarFocused()` so typing in the editor body does not jump back to the find input | 2026-08-10 |
...
| `focus-mode-caret-color.md` | Insertion caret inherits dimmed block `color`; set `caret-color: var(--canvas-ink)` on `.ql-editor.is-focus-mode` | 2026-05-31 |
...
| `fallow-false-positives.md` | Fallow can't see runtime wiring (Electron preloads, HTML tags, ref-dispatch, CSS assets). Triage dead-code reports with 4 grep patterns instead of reading every file | 2026-06-06 |
| `help-window-localstorage-isolation.md` | Each Electron `BrowserWindow` has its own `localStorage`; cross-window preferences need a get/set IPC pair executed against the main window's `webContents`, never a direct `localStorage` call from the help page | 2026-06-06 |
