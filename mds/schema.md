...
This file defines the organization, purpose, and content rules for every folder in `mds/`.
...
- Do not create new files directly in `mds/` root — use subfolders.
...
- If an archived feature needs new work, create a new plan in `mds/plan/` and reference the archived one.
