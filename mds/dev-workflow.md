...
Before searching the codebase, read `mds/START-HERE.md` first — it provides the full bootstrap sequence.

If the hard part of the task is following behavior end-to-end rather than understanding subsystem design, also read `mds/flows/README.md`.
...
10. If a whole md implementation plan is finished, move it to `mds/plan/done`.

## Documentation requirements (mandatory)

When a change affects behavior (not only formatting) or when detecting anything in the code that contradicts existing documentation, documentation updates are required in the same task. Read the requirements in `mds/update.md`.

Use the doc types consistently:

- `mds/architecture/` for subsystem design and invariants
- `mds/flows/` for "when this happens, what runs next?"
- `mds/lessons-learned/` for counter-intuitive facts
- `mds/plan/` for active refactors or implementation slices
...
- Key gotcha: `quill.getBounds()` returns coordinates **relative to `quill.container`**, not `quill.root`. See `mds/lessons-learned/quill-getbounds-container-reference.md`.
...
- If the issue is behavioral rather than API-specific, check `mds/flows/rich-editor-typing-flow.md` before changing editor lifecycle code.
