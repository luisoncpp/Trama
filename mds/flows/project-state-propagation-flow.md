...
6. **Post-save dirty flag still true** — If `saveNow` is fire-and-forget (`void`), tests may read `isDirty` before `markPaneSaved` completes. Use `await` if downstream code needs post-save state. See `mds/lessons-learned/awaitable-save-actions.md`.
