...
For deep onboarding, read `mds/START-HERE.md` → `mds/live/current-status.md` → this file.

**Phase 3 status**: All workstreams (WS1–WS5) completed and signed off. See `mds/live/current-status.md` for completion evidence.
...
See `mds/architecture/ipc-architecture.md` for endpoint mapping and extension workflow.
...
- Run tests via `scripts/run-tests.ps1` (or `Ctrl+Shift+T` in VS Code); see `mds/dev-workflow.md` for details.
