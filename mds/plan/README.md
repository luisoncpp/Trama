...
| `document-content-session-plan.md` | Deepen markdown image / broken-image orchestration into `DocumentContentSession` (renderer) + `DiskContentAdapter` (main) |
...
- When a plan is finished, move it to `mds/plan/done/` and update all doc references.
- Plans may reference but should not restate content from `mds/architecture/` or `mds/spec/`.
- Status lines must be accurate — update when a workstream completes.
- "Current high-value next tasks" belong in `mds/live/current-status.md`, not in plans.
- After moving a completed plan to `mds/plan/done/`, update this index.
